...
public class AvailabilityController {
    ...
    private void checkOverlaps(AvailabilityBean bean) throws InvalidAvailabilityException, DAOException {
        ...
        for (Availability existing : existingList) {
            ...
            boolean notSelf = (bean.getId() <= 0) || (existing.getId() != bean.getId());
            boolean sameDate = existing.getDate().equals(bean.getDate());
    ...
    private void validate(AvailabilityBean bean) throws InvalidAvailabilityException {
        ...
        if (bean.getDate().isBefore(java.time.LocalDate.now())) throw new InvalidAvailabilityException("Cannot set availability for a past date.");
